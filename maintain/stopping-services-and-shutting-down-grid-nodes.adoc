...
NOTE: Si un seul hôte inclut plusieurs nœuds de grid, n'arrêtez pas l'hôte tant que vous n'avez pas arrêté tous les nœuds de cet hôte. Si l'hôte inclut le nœud d'administration principal, arrêtez l'hôte en dernier.


NOTE: Si nécessaire, vous pouvez xref:linux-migrating-grid-node-to-new-host.adoc[Migrer des nœuds d'un hôte Linux vers un autre] pour effectuer la maintenance de l'hôte sans affecter les fonctionnalités ou la disponibilité de votre grille.
...
. Pour le nœud d'administration principal, répétez les étapes à ,connectez-vous au nœud et ,arrêt de tous les services du nœud.
