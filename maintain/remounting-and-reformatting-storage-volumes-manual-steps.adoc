...
* Vous avez xref:reviewing-warnings-for-system-drive-recovery.adoc[Consultez les avertissements relatifs à la restauration du lecteur du système du nœud de stockage].
+
...
. [[post-installation-script-action]]. Comme le `sn-recovery-postinstall.sh` Exécution du script, surveillez la page récupération dans le Gestionnaire de grille.
+
