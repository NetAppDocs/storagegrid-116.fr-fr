...
* Vous avez passé en revue la procédure à suivre xref:planning-and-preparation.adoc[planification et préparation de l'installation]Et vous comprenez la configuration requise pour les logiciels, le processeur et la RAM, ainsi que pour le stockage et les performances.
...
* Si vous déployez un nœud dans le cadre d'une opération d'extension ou de restauration, vous disposez de la xref:../expand/index.adoc[Instructions d'extension d'un système StorageGRID] ou le xref:../maintain/index.adoc[instructions de récupération et de maintenance].
...
+
Si vous déployez un nœud dans le cadre de l'opération de restauration, suivez les instructions de la section ,étape de restauration du stockage pour ajouter de nouveaux disques virtuels, reconnectez-les à partir du nœud de grille défaillant, ou les deux.
...
+
Vous devrez peut-être remapper un port si les règles de réseau de votre entreprise limitent l'accès à un ou plusieurs ports utilisés par StorageGRID. Voir la xref:../network/index.adoc[instructions de mise en réseau] Pour les ports utilisés par StorageGRID.
