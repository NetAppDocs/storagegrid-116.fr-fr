...
IMPORTANT: Vous êtes invité à télécharger le fichier du progiciel de récupération (`sgws-recovery-package-id-revision.zip`) Après avoir cliqué sur *installer* sur la page Résumé. Vous devez xref:../maintain/downloading-recovery-package.adoc[téléchargez ce fichier] pour terminer l'installation. Les mots de passe requis pour accéder au système sont stockés dans le `Passwords.txt` Fichier, contenu dans le fichier du progiciel de récupération.
